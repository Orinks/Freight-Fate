# Sign sheet — I-10 / desert Southwest (EDITABLE)

Corridor #3. Bake input -- read/edit, then I bake onto the real legs. Facts from
public data; copy invented.

**How to edit:** `treatment:` = `billboard` | `landmark` | `skip`. `leg:` /
`at_mi:` = real leg + miles from `from` city (**at_mi are ESTIMATES**). `spoken:`
= exact words, numbers spelled out, `Billboard:` lead on billboards. `describe:`
= optional pull-in read (banked, not baked).

Built legs used: `las_cruces_nm_us -> tucson_az_us` (The Thing) and
`los_angeles_ca_us -> indio_ca_us` (Cabazon).

---

### The Thing (countdown, sign one)
- treatment: billboard
- leg: las_cruces_nm_us -> tucson_az_us
- at_mi: 60
- spoken: Billboard: What is it? The Thing? Mystery of the desert ahead. Two hundred miles to guess, and whatever you do, do not stop moving.

### The Thing (countdown, sign two)
- treatment: billboard
- leg: las_cruces_nm_us -> tucson_az_us
- at_mi: 150
- spoken: Billboard: The Thing. Still ahead. Still a mystery. Still only a dollar or two to find out. Probably. No officials of any type required or allowed. If you're an official, turn back now or hide your identity.

### The Thing (countdown, sign three)
- treatment: billboard
- leg: las_cruces_nm_us -> tucson_az_us
- at_mi: 208
- spoken: Billboard: The Thing, next exit. You have wondered for two hundred miles. Do not drive past now. You made it. Will you survive?
- describe: The Thing is a roadside mystery museum near Benson that has teased drivers with "What is it?" signs for generations. Pay a dollar or two, walk a gauntlet of desert oddities, and finally meet the Thing itself. Gather yourself and contemplate what it really is -- we won't spoil it, and honestly, we don't even know.

### Cabazon Dinosaurs
- treatment: billboard
- leg: los_angeles_ca_us -> indio_ca_us
- at_mi: 82
- spoken: Billboard: Giant dinosaurs ahead at Cabazon. A brontosaurus with a gift shop in its belly and a tyrannosaurus glaring at the freeway. You have seen them in the movies, experience them today.
- describe: The Cabazon dinosaurs -- a hundred-foot Apatosaurus named Dinny with a gift shop inside, and a towering tyrannosaurus -- were built by hand over years beside the freeway. You have seen them in Pee-wee's Big Adventure, now you can see them for yourself.

### Hadley's Date Shakes
- treatment: billboard
- leg: los_angeles_ca_us -> indio_ca_us
- at_mi: 96
- spoken: Billboard: Date shakes ahead at Hadley's. Yes, a milkshake made of dates. Trust the desert. It knows what it is doing.

### San Gorgonio Windmills
- treatment: billboard
- leg: los_angeles_ca_us -> indio_ca_us
- at_mi: 65
- spoken: Billboard: A forest of windmills ahead in the San Gorgonio Pass. Thousands of them, turning in the same wind that is currently shoving your trailer around. They will not kill your birds ...or your fuzzy dice, but they'll spin, that's for sure.

---

## Held / notes
- Both Cabazon signs cluster around at_mi eighty to eighty-eight; runtime spacing
  were safe (2-mile min), but spread anyway for pacing: sixty-five / eighty-two / ninety-six.
- More I-10 candidates on other legs once those are confirmed built: Picacho Peak,
  Steins ghost town, the Shady Dell.

## Sources
Roadside attraction records (The Thing / Cabazon dinosaurs / Hadley's). Facts
only; sign copy invented.
