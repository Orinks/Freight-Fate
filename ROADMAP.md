...
- [ ] Jake traction limit on slick surfaces: the jake currently retards through the drive axle with no grip cap, so it holds a snow descent as if dry. On ice a hard jake should be able to break the drive wheels loose (trolley-jackknife risk) -- belongs to the traction deep-dive with hydroplaning and ice states.
