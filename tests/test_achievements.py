...
def test_event_achievement_speaks_through_screen_reader(monkeypatch):
    ...
    try:
        ...
        screen_reader = []
        events = []
        monkeypatch.setattr(app.ctx, "say", speech_stub(screen_reader))
...
def test_delivery_settlement_awards_only_first_delivery_on_a_first_run(monkeypatch):
    ...
    try:
        ...
        # The rookie chain's delivery-count floors (>=2/3/4) keep these off a
        # first-ever delivery even though the run would otherwise qualify.
