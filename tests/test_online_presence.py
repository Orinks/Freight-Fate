...
def _a_freshly_extracted_copy(monkeypatch, tmp_path, name: str) -> None:
    """Point the game at an empty save folder, as unzipping a new build does.

    The secret store is deliberately left alone: it belongs to the computer,
    not to the copy, which is the whole point of the recovery under test.
    """
    monkeypatch.setenv("FREIGHT_FATE_DATA_DIR", str(tmp_path / name))
    _next_session()
    online_presence.OnlineIdentity._machine_lookup_done = False


def test_a_freshly_extracted_copy_adopts_the_sign_in_this_computer_has(monkeypatch, tmp_path):
    """Unzipping a new build must not cost the player another computer.

    The Driver ID lives in the save folder, which is per copy on purpose, so
    every extracted build used to start out a stranger and activate again --
    and each activation minted another row on the account's computer list. A
    tester who unzips a build a week reached the ten-computer cap with one PC
    (armstrong445, 2026-08-15).
    """
    identity = OnlineIdentity(driver_id="road-star-abcd1234", driver_token="s" * 68)
    identity.save()

    _a_freshly_extracted_copy(monkeypatch, tmp_path, "next-build")

    assert OnlineIdentity.load() == identity
    # And it wrote its own file, so the adoption happens once rather than on
    # every launch -- with the secret still out of the save folder.
    payload = json.loads(OnlineIdentity.path().read_text(encoding="utf-8"))
    assert payload == {"driver_id": "road-star-abcd1234"}


def test_a_copys_own_identity_file_beats_the_one_the_computer_remembers(monkeypatch, tmp_path):
    """Two copies deliberately connected to two accounts stay that way."""
    OnlineIdentity(driver_id="road-star-abcd1234", driver_token="s" * 68).save()
    other = OnlineIdentity(driver_id="night-hauler-99887766", driver_token="t" * 68)

    _a_freshly_extracted_copy(monkeypatch, tmp_path, "second-copy")
    other.save()  # this copy connects an account of its own
    _next_session()

    assert OnlineIdentity.load() == other


def test_a_computer_with_no_sign_in_asks_the_store_once_not_once_a_frame(monkeypatch, tmp_path):
    """The miss is the common case, so it is the one that has to be cheap.

    Online-menu labels call load() while the screen is drawn, several times a
    frame; on Linux every one of those would be a D-Bus round trip.
    """
    reads = []
    store = FakeKeyring()
    get_password = store.get_password
    ...
    monkeypatch.setattr(store, "get_password", counted)
    monkeypatch.setattr(online_presence, "keyring", store)
    _a_freshly_extracted_copy(monkeypatch, tmp_path, "never-connected")

    assert [OnlineIdentity.load() for _ in range(20)] == [None] * 20
    assert len(reads) == 1


def test_a_fresh_copy_without_a_secret_store_just_has_no_identity(monkeypatch, tmp_path):
    monkeypatch.setattr(online_presence, "keyring", None)
    _a_freshly_extracted_copy(monkeypatch, tmp_path, "no-store")
