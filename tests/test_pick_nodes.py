...
def _load_pick_nodes():
    spec = importlib.util.spec_from_file_location("pick_nodes", ROOT / "tools" / "pick_nodes.py")
...
def test_rank_candidates_excludes_existing_and_nearby():
    ...
    out = pick_nodes.rank_candidates(
        rows, {"Chicago"}, [(41.88, -87.63)], top_n=10, min_population=100_000, dedupe_mi=30.0
    )
