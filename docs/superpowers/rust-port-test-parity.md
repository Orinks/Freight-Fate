...
(the fifteen `feat/career-1.9` commits carried into the port).
...
| live Rust namesake | 3062 | 2743 |
| `#[ignore]`d Rust stub only | 313 | 334 |
| no Rust namesake | 845 | 1116 |

The Python suite grew by 27 cases in the merge. Live Rust
coverage moved +319, ignored-only stubs -21, and
cases with no Rust namesake at all -271. Live parity is now
72.6 percent of the Python suite, up from 65.4 percent.
...
| `tests/test_curve_management.py` | PARTIAL | 46 / 0 / 51 | `crates/ff-core/tests/data_curve_management.rs`, `crates/ff-core/src/data/curves.rs` | live +8, Python +13, PORTED -> PARTIAL |
...
### `tests/test_curve_management.py` -- 5 of 51
...
- `test_nothing_read_concludes_nothing`
- `test_the_corridor_is_a_sanity_bound_not_a_decision`
...
- The merge brought the Python suite from 4193 to 4220 cases across eight
  ...
  `test_the_corridor_is_a_sanity_bound_not_a_decision`. They are pure
  `ff-core` data rules against `crates/ff-core/tests/data_curve_management.rs`,
  so they need nothing but writing.
- Live Rust coverage moved 2743 -> 3062 (+319) and files with nothing live at
  all fell from 25 to 6. The transcript suites, five pure-`ff-core` files, the
...
1. **`tests/test_curve_management.py` -- 5 cases**, the connector-class rules
   the merge added. Pure `ff-core` data against
   `crates/ff-core/tests/data_curve_management.rs`, no app involved, and the
   only place this sweep went backwards. Cheapest thing on the list, and it
   puts a file back into PORTED.
2. **`tests/test_lane_discrete.py` -- 44 cases.** Verified unblocked: the file
   drives a real `DrivingState` over a Buffalo-Rochester route, which is
   exactly what `states_driving_turns.rs` already does. Wants its own
   `states_driving_lanes.rs`. Closed-lane policing, hazard dodges, keep-right
   pressure and exit-lane gating -- the largest single block of player-facing
   behavior with nothing behind it.
3. **`tests/test_troopers.py` -- 34 stubs**, all in
...
7. **The 63 "unblocked: ... not written yet" stubs** scattered through
...
8. **`tests/test_driving_features.py` -- 116 missing.** The largest gap by
