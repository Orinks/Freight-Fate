...
Date: 2026-09-21. Status: approved in conversation, awaiting spec review.
...
- The menu rotation and the Roadhouse request the next piece one track
  ...
  `register_generated_sound("music/synth_<style>_<seed>", ...)`, then
  reports the duration so the rotation times the track correctly.
...
- At most two rendered pieces are kept registered; older ones are released.
...
- Replacing or filtering the other radio stations.
