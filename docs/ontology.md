...
| A career's copy on orinks.net, and the act of sending it there | backed up / backup -- "<career> is backed up", "Backed up to the cloud"; the by-hand row on each career's Cloud backup screen is "Back up this career now" | synced, uploaded, saved to the cloud, cloud save (the *menu* is Cloud backup, but a career is never "a cloud save") | `cloud_saves.backup_status`, `cloud_saves.recovery_status`, `city._backup_outcome_text` |
