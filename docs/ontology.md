...
| The 11-hour clock that limits time at the wheel | driving allowance; "driving available" in a report | driving time left, drive clock, hours left, hours remaining | `HosClock.drive_time_summary` |
...
| The point after coming on duty when driving must stop | legal driving cutoff; "14-hour driving window" in detailed help | duty window, shift deadline, shift window, on-duty clock | `HosClock.duty_min` |
...
| When the load is due | delivery deadline; "delivery due" in a report | deadline without "delivery", due time, drop time (the appointment is the named hour) | `Job.deadline_game_h` |
...
**The hours clocks are separate nouns, and they never share a first word.**
"At the wheel" is time already driven, "driving allowance" is what remains of
the 11-hour clock, and the "legal driving cutoff" is when the 14-hour driving
window ends. The dedicated keys lead with those nouns so a driver knows which
key landed before any number arrives; "hours left", "hours driven", and "hours
until a break" all open on "hours" and are exactly what not to say.
