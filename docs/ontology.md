...
| The lane past the exit gore, beside the highway, where a truck brakes from road speed for the ramp | deceleration lane (manual and help text; the drive itself names the speed, not the lane) | exit lane (that is the right lane the truck moves into BEFORE the gore), slip lane, off lane | `driving_events/decel_lane.rs`, `ExitRampLayout::decel_mi`, `deceleration_lane_mi` |
| The bend at the end of the deceleration lane that the exit speed is for ("Ramp curve, too fast. Slow to 30 miles per hour.") | ramp curve | ramp bend, loop, the curve (unqualified, on a ramp) | `DrivingState::ramp_curve_radius_ft`, `ExitRampLayout::curve_mi` |
...
| The ramp's advisory, the speed its curve is built for, said once as the truck enters the deceleration lane ("Exit speed 45.") | exit speed | ramp speed, ramp limit, safe ramp speed, advisory (as the spoken noun) | `armed_ramp_mph`, `take_the_ramp`, `ExitRampLayout::curve_mph` |
...
| A signal OSM maps at an intersection on a facility's streets, played by the ramp terminal's rules. Named only when it owes the truck a stop; one the truck goes through on green is the green light cue alone | traffic light ("Traffic light ahead. Light red.") | signal, stoplight, street light, ramp light | `driving_events/street_controls.rs`, `street_control_kind == "signal"`, `street_light_owes_stop` |
...
| The public road from a road stop's exit ramp to its driveway, in the manual and the docs; spoken by the street's own name, like a facility's (the row above) | access road | frontage road (unless the road is named one), service road (that is the driveway's), approach, access road zone | `STOP_STREET_ZONE`, `begin_stop_chain` |
