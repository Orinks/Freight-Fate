...
| Setting | Purpose |
| --- | --- |
| Driver profile / Share online | Your driver profile is your Orinks account sign-in for online features: the drivers board, factual road journal, achievement and profile summary sharing, and cloud save backup. The game reads exactly what can be shared before setup. The website requires a separate renewed opt-in before expanded profile data is public. That data is limited to eligible completed deliveries, official achievements, occurrence times, career level and totals, current truck, and clearly labeled last-saved city and snapshot time. Public profiles can appear in the updates feed; unlisted profiles are visible to anyone with their link; private profiles expose none of it. The game never publishes the full save, money, coordinates, active cargo details, real name, or precise live location. Turn sharing off at any time; posting stops quietly and expanded data is hidden when website consent is withdrawn. |
