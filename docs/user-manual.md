...
| Setting | Purpose |
| --- | --- |
| Driving speech | How much the road tells you, on a ladder from talkative to quiet. Coaching explains technique as you drive. Standard is the working default. Quiet cuts confirmations and status readouts to short sounds instead of sentences, keeping the safety calls, what things cost, and the turn you actually have to take as words. Urgent only keeps that same short set of words and drops the coaching too. Billboards, place names, and landmarks are not part of this ladder and keep their own switches below, so a quiet cab can still drive through a talkative countryside. Was called Speech verbosity before 1.9; a saved terse choice now lands on Quiet, and normal lands on Standard. |
