...
| Setting | Purpose |
| --- | --- |
| Profile sharing | One optional public setting covers the drivers board, eligible profile details, official achievements, automatic fictional road-journal posts, and the updates feed. It is off until you connect your orinks.net driver and turn it on. The game never publishes the full save, money, coordinates, active cargo details, real name, or precise live location. Turning it off stops local posting immediately; the game confirms when orinks.net has made the shared profile private. If confirmation fails, the menu says off requested; choose it again when connected to retry. |
