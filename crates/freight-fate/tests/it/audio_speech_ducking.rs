...
fn test_the_backends_scale_engine_weather_and_music_only() {
    ...
    assert!((ducked.category_volume(Category::Engine) - engine * SPEECH_DUCK_LEVEL).abs() < 1e-9);
    assert!((ducked.category_volume(Category::Weather) - weather * SPEECH_DUCK_LEVEL).abs() < 1e-9);
