...
fn test_mapped_state_lines_are_authoritative_in_delivery_transcripts() {
    for (cities, state, passing_city, expected_crossings) in [
        ...
        assert!(result.transcript_text().contains(&passing_phrase));
        assert!(!result
            .transcript_text()
            .contains(&format!("Crossing into {state}. Passing {passing_city}")));
        if expected_crossings > 0 {
            let boundary = result
                .transcript
                .iter()
                .position(|line| line.contains(&format!("Crossing into {state} near ")))
                .expect("the mapped boundary was announced");
            let passing = result
                .transcript
                .iter()
                .position(|line| line.contains(&passing_phrase))
                .expect("the city passing line was spoken");
            assert!(boundary < passing, "{}", result.transcript_text());
        }
    }
}
