...
// -- TEMPORARY stubs for the methods the mixin modules provide ------------------------------
//
// Each `impl DrivingState` block below belongs to one mixin task. When that
// task lands its real `impl DrivingState { ... }` in its own file, it DELETES
// its block here (a duplicate method is a compile error that points straight
// at the pair). Every stub does nothing or returns a neutral default and logs
// once per process, so the crate builds and a drive can be constructed,
// entered and ticked before the mixins exist.
//
// Signatures are the contract: keep them when porting.
pub mod pending {
    use std::sync::Once;
    ...
    macro_rules! pending_once {
        ($name:literal) => {{
            ...
            ONCE.call_once(|| log::warn!("driving: {} is a pending stub", $name));
        }};
    }

    // -- provided by driving_controls.rs (DrivingControlsMixin) ----------------------------
    impl DrivingState {
        /// `handle_event(event)`: KEYDOWN/KEYUP under `ctx.player_asked`.
        pub fn handle_key_event(&mut self, _ctx: &mut GameContext, _event: &InputEvent) {
            pending_once!("handle_key_event");
        }

        /// `handle_controller(event, manager)`.
        pub fn handle_controller_event(&mut self, _ctx: &mut GameContext, _event: &InputEvent) {
            pending_once!("handle_controller_event");
        }

        /// `on_controller_disconnect()`: pushes the pause menu.
        pub fn handle_controller_disconnect(&mut self, _ctx: &mut GameContext) {
            pending_once!("handle_controller_disconnect");
        }
    }

    // -- provided by driving_updates.rs (DrivingUpdateMixin) --------------------------------
    impl DrivingState {
        /// `update(dt)`: the per-frame update. MUST call
        /// `ctx.update_music_rotation(dt)` (the Python `super().update(dt)`).
        pub fn update_frame(&mut self, ctx: &mut GameContext, dt: f64) {
            pending_once!("update_frame");
            ctx.update_music_rotation(dt);
        }

        /// `tick_covered_music(dt)`: keep the drive's playlist turning over
        /// under a menu.
        pub fn tick_drive_music(&mut self, _ctx: &mut GameContext, _dt: f64) {
            pending_once!("tick_drive_music");
        }

        /// `apply_radio_settings_now()`.
        pub fn apply_radio_settings_to_drive(&mut self, _ctx: &mut GameContext) {
            pending_once!("apply_radio_settings_to_drive");
        }

        /// `_play_radio_current()`: (re)start the tuned station on entry.
        pub fn play_radio_current(&mut self, _ctx: &mut GameContext) {
            pending_once!("play_radio_current");
        }

        /// `_apply_radio_volume()`: music volume = radio setting x signal x ducks.
        pub fn apply_radio_volume(&mut self, ctx: &mut GameContext) {
            pending_once!("apply_radio_volume");
            let volume = ctx.settings.radio_volume
                * self.radio_signal_factor
                * self.radio_picket_duck
                * self.radio_cue_duck;
            ctx.audio
                .set_volumes(&crate::audio::VolumeUpdate::default().music(volume));
        }

        /// `_start_playlist_station(station, fade_ms=900, advance=False)`.
        pub fn start_playlist_station(
            ...
            pending_once!("start_playlist_station");
        }

        /// `_start_station_rotation(station, fade_ms=900)`.
        pub fn start_station_rotation(
            ...
            pending_once!("start_station_rotation");
        }
    }

    // -- provided by driving_pickup.rs (DrivingPickupMixin) -----------------------------------
    impl DrivingState {
        ...
        pub fn pickup_progress_summary(&self, ctx: &GameContext) -> String {
            // Spoken distances go through the unit setting; a player on metric
            // must not hear miles here just because this handler moved modules.
            let s = &ctx.settings;
            format!(
    ...
    impl DrivingState {
        ...
        pub fn cancel_cruise(&mut self, _ctx: &mut GameContext, _preserve_session: bool) {
            pending_once!("cancel_cruise");
        ...
        pub fn restore_speed_control_session(
            ...
            pending_once!("restore_speed_control_session");
            self.speed_control_armed = armed;
            self.speed_control_target_mph = target_mph;
        }
    }

    // -- provided by driving_liquid.rs (LiquidLoadMixin) ------------------------------------
    impl DrivingState {
        /// `_stop_liquid_cues()`.
        pub fn stop_liquid_cues(&mut self, _ctx: &mut GameContext) {
            pending_once!("stop_liquid_cues");
        }
    }
}
